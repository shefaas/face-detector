...
def cropFace(detections, args, image):
	croppedFaces = []
	(h, w) = image.shape[:2]
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			croppedFaces.append(croppedImage)

	return croppedFaces


def galleryImage(croppedFaces):

	# assuming croppedFaces has an even number of images and the one image size
	galleryImageWidth = len(croppedFaces) * croppedFaces[0].shape[:2][0]
	galleryImageHeight = len(croppedFaces) * croppedFaces[0].shape[:2][1]
	galleryImageSize = galleryImageWidth, galleryImageHeight, 3
	galleryImage = np.zeros(galleryImageSize, dtype=np.uint8)

	for i in range(len(croppedFaces)):
		row = 0
		col = 0
		image = croppedFaces[i]

		# TODO adding faces into galleryImage
...
resizedImage = cv2.resize(image, (int(image.shape[:2][1]/2), int(image.shape[:2][0]/2)))
#drawBoxes(detections, args, resizedImage)

croppedFaces = cropFace(detections, args, image)

galleryImage = galleryImage(croppedFaces)

# show the output image
cv2.imshow("Gallery Image", galleryImage)
cv2.waitKey(0)